...
export default function CallCapture({ clientId, time, carer, state }: { clientId: string; time: string; carer: string; state: string }) {
  ...
  async function act(action: "checkin" | "checkout" | "undo") {
    setBusy(true);
    try {
      const res = await fetch("/api/ecm", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action, clientId, schedTime: time, carer }),
      });
  ...
  if (state === "onsite") {
    return (
      <div className="flex" style={{ gap: 6, justifyContent: "flex-end" }}>
        <button className="btn btn-primary" style={sm} disabled={busy} onClick={() => act("checkout")}>Check out</button>
