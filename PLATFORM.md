...
2. **CRM core** — ✅ **built.** `clients` + `pii_access_log` tables; Client register (`/clients` — status chips, search, PII-masked names); **editable** client profile (`/clients/[id]` — identity, care package, schedule, plus the **Schedule of Service as a weekly overview** — a roster-style Mon–Sun grid of the client's permanent calls; a **coordinator reassigns a carer by requesting a permanent change** (goes to the CSM as a `permanent_change_requests`, shown "awaiting CSM"), while a **CSM/manager reassigns directly** and approves/declines pending requests inline (`set_schedule_carer`); CSM/managers also get an "Edit plan structure" editor to add/remove calls and change times/tasks; saving recomputes the care team — **editable care-plan tasks by domain**, a DB-backed **care notes/diary** (`care_notes`, category + tone) and a **documents register** (`client_documents`, status + expiry), all via `/api/clients/[id]` and audited); **PII reveal-gate** (`/api/pii/reveal`) + **access log** (`/access-log`, oversight-only). Seeded from the prototype `buildClients()` (`data/qms-clients.json`).
