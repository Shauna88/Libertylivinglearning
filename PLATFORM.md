...
- **CRM** — Cover board (today's gaps/red triage), Live monitor (visit states), **Client register** (grouped **By area** with card view + Table toggle, area & status facets, search, PII gate; each client card shows **delivered vs planned hours this week** (covered scheduled hours vs the full plan — amber when short, and 0 delivered while the client is paused in hospital / on hold) alongside contracted hours and flags **N unassigned calls**, an urgent ringed **"unassigned call in the next 24 hours"** flag (with time-to-start) for imminent uncovered calls, and **new diary notes** (last 48 hours) at a glance → client profile: overview / schedule / care plan tasks by domain / care notes / documents / carers / flags), **New referral intake** (`/clients/new`), Scheduling + Daily/Carer roster + By area (cover overrides → CSM approval), **Carer directory** (`/carers` — DB-backed carer records with home area, travel radius, skills, hours; add/edit; powers carer-matching; **open a carer → `/carers/[id]`** for their profile + hours/clients summary + full **working week** across every client — cards, table, and an **Availability** timeline showing each day's booked calls vs open gaps (07:00–22:00) with free-time totals — and **unassigned client calls in the carer's travel radius that land in a gap appear as clickable markers you assign the carer into** — this week only (cover, no approval) **or permanently** (an approver rewrites the base Schedule of Service; a coordinator sends a request to the CSM), which then reconciles across their week, hours and clients. On the **client side**, every unassigned call shows a **"free & nearby now"** row — carers who cover that area and have no clashing call, ranked by spare hours, one click to assign), Call log, Bulk import (CSV + column mapping + template), **PII gate**.
...
- **Compliance & assessments hub** (`/compliance`, CRM + workforce + oversight — so the **Director of Quality** sees everything in one place) — a two-tab oversight view: **Carer credentials** (Garda vetting, mandatory training, right-to-work, insurance per active carer; expired / expiring (≤45d) / missing; safety-critical lapses mark a carer **"not cleared to roster"**) and **Client assessments & reviews** (each client's care-plan review + risk assessments, overdue / due-soon / not-done, by SU so no client PII). Editing stays on the carer/client profile (`carer_compliance`, `client_assessments`); dashboards prompt on lapses.
...
- **Structured assessments & care-plan reviews** — every client carries an initial needs assessment plus recurring risk assessments (care-plan review, falls, moving & handling, environment/PEEP, medication, skin integrity, nutrition) with review cadences (`client_assessments`); the **client profile** shows each with a status (in date / due soon / overdue / not done) and an editable next-review date, and the CSM / coordinator dashboards prompt when a client has a review overdue or due soon.
- **Unified client activity timeline** — an **Activity** section on the client profile aggregates every event on the record into one chronological, filterable feed (`clientActivity()`): care notes, call events, cover changes, permanent-change requests, documents, assessments, ECM check-ins and PII reveals — the CRM "single view" of a client.
- **Finance** — overview, client invoicing, rate schemes, HCA pay & hours.
- **Service status bar** — a slim, shared strip at the top of every office/department login (`serviceVitals()` in the `(app)` layout) showing the same vital signals — uncovered calls, missed check-ins, carers not cleared to roster, care-plan reviews overdue, open safeguarding / complaints / incidents — so no department misses anything. One source, every login; self-hides when all-clear; front-line/portal/demo keep their own focused views.
- **Notifications** (`/notifications`, CRM + workforce + oversight) — outbound **SMS / email** to carers & clients from templates (roster published, visit reminder, shift offer, welfare follow-up, review due), with `{placeholder}` fills, a recipient picker (carers / clients / groups) and a **delivery log** (`notifications`). Messages are **queued** — actual SMS/email delivery needs a gateway (Twilio / SendGrid) the developer connects, so nothing is silently dropped.
- **Analytics & funder reporting** (`/analytics`, oversight + finance) — service KPIs (active clients, care delivered vs planned this week, uncovered calls, carer utilisation, open complaints/incidents, billed & margin), care-delivered-by-area bars, and a **funder report** (hours & billing split by HSE CHO / Fair Deal / private) with a per-CHO rate-scheme breakdown, computed live from schedules × rate cards (`lib/analytics.ts`).
...
1. **9-role foundation** — ✅ **built.** `lib/roles.ts` is the single source of truth: each role has a **capability set** (`crm` / `oversight` / `finance` / `recruit` / `improvement` / `workforce`), a **department**, and a **hub scope**. The gate arrays (`CRM_ROLES`, `OVERSIGHT_ROLES`, `FINANCE_ROLES`, `RECRUIT_ROLES`, `IMPROVEMENT_ROLES`, `WORKFORCE_ROLES`) derive from it; the sidebar is capability-driven with a **role-aware hub label** ("HR Management" vs "Quality Management"); the **Improvement hub is department-scoped** (Quality/Exec/CSM see every open issue, Director of HR sees only HR-routed issues — verified: routing an incident to HR makes it appear in the HR director's inbox); and the dashboard shows each role's remit, quick-links and a department issue inbox. Dashboards are **personalised per role** — senior roles get an HSE scorecard, the Director of Finance a live financial summary, coordinators a "your day" operational view, and the **CSM an operational cockpit**: a ranked **To-do** list plus tiles and sections for **permanent carer changes awaiting approval, uncovered calls today, sick / no-show carers to re-cover, complaints & incidents to follow up, and care-plan reviews due** (the HSE scorecard is kept below). The To-do list is **interactive** (`TodoBoard`, `user_todos`): the system prompts are auto-generated and **click through to the outstanding tab**, and every login can **add their own to-dos and tick them off** (with a completed/strike-through state and delete) right alongside them. A **quick-add "Common tasks" dropdown** offers department-relevant templates (`lib/todopresets.ts` — e.g. schedule a spot check, complete care-plan reviews, schedule supervision, send invoices), and a to-do can be **shared with another department** to collaborate: it appears on that department's dashboards tagged "from {name}", and the sender tracks it tagged "→ {dept}" (`user_todos.to_dept`). A to-do (and a message) can also be tagged with a **"relates to"** context — a client or a carer/HCA (`lib/refs.ts`, `ref_label`/`ref_href`) — shown as a clickable chip so a shared item carries who/what it's about. The **compose message** modal is a fixed head/body/footer layout so **Send stays visible** without scrolling. Every management role now leads with a **ranked prompt list of what needs them today**: department heads get **Daily improvement prompts** (open issues to review & sign off, QIP actions overdue / in progress, training pushes out — department-scoped for the HR director), the **On-Call Manager** an out-of-hours cockpit (uncovered-now, sick / no-show to re-cover, calls to follow up, **paused clients to watch for resume**), the **Care Coordinator** a to-do-today list, and the **Director of Finance** finance prompts (unpaid invoices, margin watch). One demo login per role is seeded.
2. **CRM core** — ✅ **built.** `clients` + `pii_access_log` tables; Client register (`/clients` — status chips, search, PII-masked names); **editable** client profile (`/clients/[id]` — identity, care package, schedule, plus the **Schedule of Service as a weekly overview** with a **Cards / Table view toggle** (the Table view is a roster-style Time × Mon–Sun grid). **Temporary changes need no approval:** a coordinator can **reassign a call to another carer for this week only**, or **unassign it with a required reason** (sick / no-show / cancelled / hospital / reallocating cover / staffing change / other) — both applied via `cover_assignments` and reverting to the base plan next week. **Only a permanent (ongoing) carer change needs CSM sign-off:** a coordinator sends a request with a justification (`permanent_change_requests`, shown "awaiting CSM") and the CSM approves/declines inline; an approval rewrites the base Schedule of Service (`set_schedule_carer`) and clears the one-week cover slot. **Carer matching** ranks the carer directory (`data/qms-carers.json`, `lib/carers.ts`) for each client by **travel radius**, **skills matched to the client's conditions**, and **hours free this week**, with reasons/warnings; the best-matched carers surface as a "Suggested carers" card and as a "Suggested" group (with % match) in every carer dropdown. A client with **no schedule yet** gets a "Set up the original Schedule of Service" prompt with the structural editor opened; CSM/managers also get an "Edit plan structure" editor to add/remove calls and change times/tasks — with a roster-style **Week grid** view (all seven days as columns, with a "copy this day to all days" shortcut) alongside the detailed Day view, so setting up a new client's original schedule mirrors the weekly overview; saving recomputes the care team — **editable care-plan tasks by domain**, a DB-backed **care notes/diary** (`care_notes`, category + tone) and a **documents register** (`client_documents`, status + expiry), all via `/api/clients/[id]` and audited); **PII reveal-gate** (`/api/pii/reveal`) + **access log** (`/access-log`, oversight-only). Seeded from the prototype `buildClients()` (`data/qms-clients.json`).
...
7. **Front-line & cross-team** — ✅ **built.** *Raise a concern:* front-line staff (Healthcare Assistants) can **log** complaints / incidents / safeguarding but only **see their own** reports (`RegisterScreen` filters to `reporter_id` for non-management; the nav group becomes "Raise a concern" and the org-wide open badges are suppressed) — management still sees the full register. *Time off:* `/time-off` (`time_off_requests`) — any login requests annual / unpaid / sick / parental leave and tracks its status; approvers (oversight + workforce roles) get a pending queue with approve / decline, surfaced as a dashboard prompt. *Messaging:* `/messages` (`messages`) — every login has a department mailbox (`messagingDept`); compose a **message or meeting request** to any department (or all staff), with an inbox / sent split and reply. A meeting request carries a real date/time (picker) and renders **"Add to Microsoft calendar"** (Outlook deep link) + a **downloadable .ics** invite on the received message. *Master log:* `/registers` — a management-only consolidated **governance master log** of every complaint / incident / safeguarding entry in one filterable table (type / status / search → open the register to sign off). **Audits & QIP** (`/audits`) is scoped to **Quality Management** (org-wide quality scope) — the HR director no longer sees it. All via `/api/time-off` and `/api/messages`, audited. *(True two-way Microsoft 365 calendar sync needs the developer to connect Graph API OAuth; the app ships the standard add-to-calendar deep link + .ics, which works with Outlook and any calendar.)*
8. **GDPR hardening** — 🟡 **partly built.** `/data-protection` (oversight) now provides: a general **`audit_log`** (logins, PII reveals, issue sign-offs, training/SOP assignments, register + call events — best-effort, never breaks the primary action) with a viewer; **DSAR export** (`/api/dsar/[clientId]` — full per-client JSON bundle: record, related call events and access history, logged to audit + PII log); the **access register** (each login × the data classes it can reach, from `ROLE_DATA_ACCESS`); and the **retention schedule** (`lib/retention.ts`). DPO contact + safeguards shown. *Still operational/roadmap: 2FA for manager/director logins, encrypted EU-region backups + restore test, annual access review, automated retention purge, and DPO sign-off of the policy.*
