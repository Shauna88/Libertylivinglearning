...
import { CRM_ROLES, WORKFORCE_ROLES, getCarer, addCarerDoc, type Role } from "@/lib/db";
...
/** Upload a PDF onto a carer's (HCA) record (multipart/form-data: file, name?, status?, expiry?). */
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const row = await addCarerDoc({
    carerId: id, name, status, expiry, addedBy: session.user.name ?? "Coordinator",
